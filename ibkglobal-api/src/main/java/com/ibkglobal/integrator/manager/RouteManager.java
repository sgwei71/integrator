...
public class RouteManager {
	...
	public void initBuilder() {
		instanceAdmin.getAllRouteInfo().stream().forEach(routeInfo -> {
			try {
				...
				} else if(routeInfo.getRouteCreate().get() instanceof RestDefinition ){
					logger.debug("route info  -> {}",routeInfo);
					logger.debug("RouteCreate -> {}",routeInfo.getRouteCreate());
					RestDefinition restDefinition=(RestDefinition)routeInfo.getRouteCreate().get();
					List<RouteDefinition> asRouteDefinitions = restDefinition.asRouteDefinition(camelConfig.getCamelContext());
					camelConfig.getCamelContext().addRouteDefinitions(asRouteDefinitions);
				}else {
	...
	public void updateBuilder(RouteInfo routeInfo) {
