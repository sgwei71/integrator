...
import com.ibkglobal.integrator.engine.bean.api.error.ErrorCatchAPI;
...
public abstract class RouteCreateDefault extends RouteDefinition implements RouteCreate {
	 ...
	 * On API Exception
	 */
	protected void onAPIException() {
		onException(Exception.class).handled(true)
			.bean(ErrorCatchAPI.class,"catchError")
			.end();
	}
	/**
	 * On MCA Exception
	...
	protected void setDefaultHeader(String seq) {
		...
		this.setHeader(ConstantCodeAPI.ORG_CODE, Builder.constant(builderInfo.getOrgCd()));
		this.setHeader(ConstantCodeAPI.BIZ_CODE, Builder.constant(builderInfo.getBizCd()));
		this.setHeader(ConstantCodeAPI.SYS_CODE, Builder.constant(builderInfo.getSysCd()));
		this.setHeader(ConstantCodeAPI.PARSING_TYPE,Builder.constant(builderInfo.getParsingType()));
		this.setHeader(ConstantCodeAPI.INSTANCE_ROUTE_TYPE,Builder.constant(builderInfo.getInstanceRouteType()));
		this.setHeader(ConstantCodeAPI.INSTANCE_TYPE,Builder.constant(builderInfo.getInstanceType()));
		this.setHeader(ConstantCodeAPI.LOG_NAME, Builder.constant(builderInfo.getLogName()));
	}
